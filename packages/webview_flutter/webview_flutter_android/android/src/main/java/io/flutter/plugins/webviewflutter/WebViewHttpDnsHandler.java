...
public abstract class WebViewHttpDnsHandler {
    static final String TAG = "WevView.Flutter.CustomWebViewClient";

    @Nullable
    public abstract String getIp(String host);


    @Nullable
    public WebResourceResponse shouldInterceptRequest(WebView view, WebResourceRequest request) {
        String scheme = Objects.requireNonNull(request.getUrl().getScheme()).trim();
            ...
            try {
                ...
                if (TextUtils.isEmpty(mime)) {
                    ...
                    if (!TextUtils.isEmpty(charset) || (isBinaryRes(mime))) {
                        ...
                        return new WebResourceResponse(mime, encoding, connection.getInputStream());
                    }
                }
            } catch (Exception e) {
                //
